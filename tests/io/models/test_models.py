...
from sweat.io.models import exceptions
...
class TestWorkoutDataFrame:
    ...
    def test_is_valid_invalid_sample_rate(self):
        data = {"time": range(0, 20, 2), "heartrate": range(10), "power": range(10)}
    ...
    def test_is_valid_invalid_min_value(self):
        data = {"time": range(10), "heartrate": range(-10, 0), "power": range(10)}
        wdf = dataframes.WorkoutDataFrame(data)
        wdf = wdf.set_index("time")
    ...
    def test_is_valid_invalid_max_value(self):
        data = {"time": range(10), "heartrate": range(10), "power": range(10000, 10010)}
        wdf = dataframes.WorkoutDataFrame(data)
        wdf = wdf.set_index("time")
    ...
    def test_compute_weighted_average_power(self, wdf_big):
        assert wdf_big.compute_weighted_average_power() == pytest.approx(
            156.24624656343036, 0.1
        )
